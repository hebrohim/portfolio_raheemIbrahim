  ...
  return (
    <div className="h-[100vh] w-full md:flex">
      <div className="flex mt-20 flex-col w-full items-center py-20 md:flex-row-reverse md:justify-around">
        <motion.div initial = {{opacity:0,x:"100vw"}} animate = {{opacity:1,x:0}} transition={{type:"spring",delay:2,stiffness:100}}>
        <motion.img drag dragConstraints ={{left:0,right:100,top:0,bottom:100}}  dragElastic ={2}
          whileHover ={{boxShadow:"0px 0px 10px black",cursor:"grab"}} src={me}
          className="w-40 h-40 rounded-full object-cover border-4 border-amber-600 p-1 md:w-60 md:h-60 md:border-4"
        />
        </motion.div>
        <motion.section initial = {{opacity:0,x:"-100vw"}} animate = {{opacity:1,x:0}} transition={{type:"spring",delay:2,stiffness:100}} className="flex flex-col justify-center items-center">
          ...
          <section className="social_media">
            <ul className="flex w-full justify-end mt-10">
              <motion.li variants={buttonAnimation} whileHover = "whileHover" transition= "transition" className="mr-2 w-8 h-8 flex items-center justify-center rounded-full border-slate-900  hover:bg-amber-600 cursor-pointer border-2 md:w-12 md:h-12 md:border-4 md:mr-5 dark:border-white">
